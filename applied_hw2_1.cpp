 ...
 *
 * File: Main interface file for all the classes of the Film Library
 */
...
static const QList<QString> listFilmTypes = QList<QString>() << "Action"
                                                             << "Comedy"
                                                             << "Sci-Fi"
                                                             << "Thriller"
                                                             << "Horror";
static const QList<QString> listMPAARatings = QList<QString>() << "G"
                                                               << "PG"
                                                               << "PG-13"
                                                               << "R";
static const QList<QString> listGrade = QList<QString>() << "Freshman"
                                                         << "Sophomore"
                                                         << "Junior"
                                                         << "Senior"
                                                         << "Graduate";
...
QStringList funcaddgeneral(FilmList *fl, bool top) {
  ...
  qout << labelTitle << flush;
  sl.append(qin.readLine());
  qout << labelDirector << flush;
  sl.append(qin.readLine());
  qout << labelFilmLength << flush;
  sl.append(qin.readLine());
  qout << labelReleaseDate << flush;
  sl.append(qin.readLine());
...
QStringList funcaddeducational(FilmList *fl, bool top) {
  ...
  qout << labelSubject << flush;
  sl.append(qin.readLine());
  qout << labelGradeLevel << "(" << listGrade.join(",") << "):" << flush;
  sl.append(qin.readLine());
  qout << labelAuthors << flush;
  sl.append(qin.readLine());
...
QStringList funcaddentertainment(FilmList *fl, bool top) {
  ...
  sl.append(funcaddgeneral(fl, false));
  qout << labelFilmTypes << "(" << listFilmTypes.join(",") << "): " << flush;
  sl.append(qin.readLine());
  qout << labelMPAARatings << "(" << listMPAARatings.join(",")
       << "): " << flush;
  sl.append(qin.readLine());
...
const QList<AddFunc> addFunc =
    QList<AddFunc>() << funcaddgeneral << funcaddeducational
                     << funcaddeducationalbystudents
                     << funcaddeducationalbypublishers << funcaddentertainment;
...
bool funcadd(FilmList *fl) {
  while (1) {
    for (int i = 0; i < filmSwitch.length(); ++i) {
      qout << QString::number(i) << ": " << filmSwitch[i] << endl;
    }
    ...
    if (ok && ((0 > choice) || (choice >= filmSwitch.length()))) {
      throw QString() + "Must choose a number [" + QString::number(0) + "," +
          QString::number(filmSwitch.length()) + ")";
    }
...
bool funcexit(FilmList *fl) { return true; }

const QList<QString> taskSwitch = QList<QString>() << "Add film"
                                                   << "Remove film"
                                                   << "Find film"
                                                   << "Get film ID"
                                                   << "Exit";
const QList<FilmFunc> taskFunc = QList<FilmFunc>()
                                 << funcadd << funcremove << funcfind
                                 << funcgetid << funcexit;
  ...
  while (1) {
    try {
      for (int i = 0; i < taskSwitch.length(); ++i) {
        qout << QString::number(i) << ": " << taskSwitch[i] << endl;
      }
      ...
      if (ok && ((0 > choice) || (choice >= taskSwitch.length()))) {
        throw QString() + "Must choose a number [" + QString::number(0) + "," +
            QString::number(taskSwitch.length()) + ")";
      }
